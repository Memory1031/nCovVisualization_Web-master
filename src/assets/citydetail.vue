<template>
  <div>
    <div id="myChart" :style="{width: '70vw',height: '500px'}"></div>
  </div>
...
<script>
  ...
  export default {
      ...
      methods:{
          init(){
              axios({
                  url: apiRoot + '/epidemic/cityQuery',
                  method: 'post',
                  data: {
                      province: '',
                      name: this.factor,
              ...
              myChart.setOption({
                  ...
                  series: [{
                      ...
                      data: [{                    // 雷达图的数据是多变量（维度）的
                          name: '女',                 // 数据项名称
                          value: [row.remainScore, row.remainCountScore, row.weekScore, row.growthScore, row.abroadInputScore],        // 其中的value项数组是具体的数据，每个值跟 radar.indicator 一一对应。
